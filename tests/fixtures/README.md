...
Status: **built.** `northgate/` ships 30 items, 22 services, and a second crawl,
generated from `northgate/spec.yaml`. `FixtureTransport` in
`src/arcgis_inventory/transport.py` is the consumer;
`tests/test_fixture.py` checks the fixture against its own claims.
...
The layer, field, and app vocabulary deliberately mirrors the municipal pattern
anyone who has taken an Esri course will recognize — parcels, zoning, storm and
sanitary sewer, hydrants, address points, trees, plow routes. The *shapes* are
familiar on purpose so the fixture reads as real work. The organization is not.
...
tests/fixtures/northgate/
  ...
  portal/
    ...
    groups.json
  search/
    page-1.json                # num=10&start=1 ... paginated, 3 pages
    page-2.json
    page-3.json
  items/
  ...
  services/
    <host>/<slug>.json         # service metadata; path flattened to one
                               # component (see below)
  expected/
    inventory.json             # golden: classification + confidence per item
    edges.json                 # golden: full dependency edge list
    endpoints.json             # golden: the non-item nodes and their access
  run2/                        # the same org a month later; overlays run 1
...
`findings.json` and `recommendations.json` from the original spec are
...
**Page size is 10, not 100.** Thirty items over three pages exercises pagination;
a single page would not. Real crawls use 100.

**Service paths are flattened** into a single filename component
(`server__rest__services__Public__Parcels__FeatureServer.json`), truncated with
a short hash past 80 characters. A real REST path is deep enough that the
natural directory tree exceeds the Windows 260-character limit — at which point
git reports "Filename too long" and skips the file. `fixture_service_filename`
in `transport.py` owns that rule, and `build.py` imports it rather than keeping
a second copy.

**`run2/` is an overlay, not a copy.** It redefines only what changed; anything
else falls back to run 1, so its diff shows the delta and nothing else.
